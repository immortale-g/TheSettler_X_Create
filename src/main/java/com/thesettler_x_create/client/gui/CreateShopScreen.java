...
public class CreateShopScreen extends AbstractContainerScreen<CreateShopMenu> {
    ...
    protected void init() {
        ...
        Button save = Button.builder(LABEL_SAVE, btn -> {
            String address = addressBox.getValue();
            PacketDistributor.sendToServer(new SetCreateShopAddressPayload(menu.getPos(), address));
        }).bounds(x + 10, y + 50, 60, 20).build();
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        if (addressBox != null && addressBox.isFocused() && addressBox.keyPressed(keyCode, scanCode, modifiers)) {
            return true;
        }
    ...
    public boolean charTyped(char codePoint, int modifiers) {
        if (addressBox != null && addressBox.isFocused() && addressBox.charTyped(codePoint, modifiers)) {
            return true;
        }
