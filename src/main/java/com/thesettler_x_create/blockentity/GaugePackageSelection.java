 ...
 * task list and a stock source, both are cheap to pin down.
 */
public final class GaugePackageSelection {
  private GaugePackageSelection() {}

  /**
   * Pulling from the shop's stock: up to {@code amount} of {@code key}, or as much of it as is
   * there. The seam a test stands in for. Where the shop takes it from, hut buffer or racks, is the
   * caller's business.
   */
  @FunctionalInterface
  ...
  /** The task that gets served and what came out of the shop for it. */
  public record Choice(GaugePackagingTask task, ItemStack extracted) {}

  /**
   * The first task the shop can cover anything of, or {@code null} when it can cover none.
   *
   ...
   * stock, it picks the same task and the same amount. That is what keeps Create from shipping a
   * preview the shop never hands out.
   */
  @Nullable
  public static Choice select(List<GaugePackagingTask> tasks, StockSource stock, boolean simulate) {
    for (GaugePackagingTask task : tasks) {
      int wanted = Math.min(task.amount(), CreatePackageBridge.capacityFor(task.item()));
      if (wanted <= 0) {
        continue;
      }
      ItemStack extracted = stock.pull(task.item(), wanted, simulate);
