...
public class VirtualCreateNetworkItemHandler implements IItemHandler {
    ...
    public ItemStack extractItem(int slot, int amount, boolean simulate) {
        ...
        // Keep reservations until the parent request completes (or TTL expires) to avoid re-requests mid-delivery.

        if (Config.DEBUG_LOGGING.getAsBoolean()) {
            TheSettlerXCreate.LOGGER.info("[CreateShop] extractItem {}x {} (simulate={})",
                    extracted.getCount(), extracted.getHoverName().getString(), simulate);
        }
