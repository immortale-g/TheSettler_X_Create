...
public class CreateShopPermaModuleWindow extends AbstractModuleWindow<CreateShopPermaModuleView> {
    ...
    private List<CreateShopPermaModuleView.OreEntry> applyFilter(List<CreateShopPermaModuleView.OreEntry> ores) {
        String filter = "";
    ...
    private void setAll(boolean enabled) {
        ...
        for (CreateShopPermaModuleView.OreEntry entry : ores) {
            ...
            PacketDistributor.sendToServer(new SetCreateShopPermaOrePayload(building.getPosition(), id, enabled));
        }
        updateOreList();
    }
