...
public final class CreateShopResolverInjector {
    ...
    public static void ensureGlobalResolver(IColony colony) {
        ...
        for (var entry : colony.getServerBuildingManager().getBuildings().entrySet()) {
            ...
            shops++;
            if (allowDebugLog) {
                TheSettlerXCreate.LOGGER.info("[CreateShop] Found shop at {}", shop.getLocation().getInDimensionLocation());
            }
            ...
            if (!registered) {
                try {
                    ...
                    TheSettlerXCreate.LOGGER.info("[CreateShop] Global resolver registration failed: {}", ex.getMessage());
                }
            }
        ...
        while (prune.hasNext()) {
            ...
            try {
                ...
                if (resolver instanceof CreateShopRequestResolver && !allowedShopResolvers.contains(token)) {
                    prune.remove();
                    pruned++;
                }
            } catch (IllegalArgumentException ignored) {
        ...
        if (injected > 0 || pruned > 0) {
            ...
            TheSettlerXCreate.LOGGER.info("[CreateShop] Global resolver tick: shops={}, injected=0", shops);
            TheSettlerXCreate.LOGGER.info("[CreateShop] DELIVERABLE list entries={}", deliverableList.size());
            for (var token : deliverableList) {
                try {
                    var resolver = resolverHandler.getResolver(token);
                    TheSettlerXCreate.LOGGER.info("[CreateShop] DELIVERABLE resolver entry: {} -> {}",
                            token, resolver.getClass().getName());
                    TheSettlerXCreate.LOGGER.info("[CreateShop] DELIVERABLE resolver detail: priority={} requestType={}",
                            resolver.getPriority(), resolver.getRequestType());
                } catch (IllegalArgumentException ex) {
                    TheSettlerXCreate.LOGGER.info("[CreateShop] DELIVERABLE resolver entry: {} -> <missing>", token);
                }
            }
            TheSettlerXCreate.LOGGER.info("[CreateShop] REQUESTABLE list entries={}", requestableList.size());
            for (var token : requestableList) {
                try {
                    var resolver = resolverHandler.getResolver(token);
                    TheSettlerXCreate.LOGGER.info("[CreateShop] REQUESTABLE resolver entry: {} -> {}",
                            token, resolver.getClass().getName());
                    TheSettlerXCreate.LOGGER.info("[CreateShop] REQUESTABLE resolver detail: priority={} requestType={}",
                            resolver.getPriority(), resolver.getRequestType());
                } catch (IllegalArgumentException ex) {
                    TheSettlerXCreate.LOGGER.info("[CreateShop] REQUESTABLE resolver entry: {} -> <missing>", token);
                }
            }
            ...
            for (var token : toolList) {
                try {
                    var resolver = resolverHandler.getResolver(token);
                    TheSettlerXCreate.LOGGER.info("[CreateShop] TOOL resolver entry: {} -> {}",
                            token, resolver.getClass().getName());
                    TheSettlerXCreate.LOGGER.info("[CreateShop] TOOL resolver detail: priority={} requestType={}",
                            resolver.getPriority(), resolver.getRequestType());
                } catch (IllegalArgumentException ex) {
        ...
        for (var entry : identitySnapshot) {
            ...
            if (Config.DEBUG_LOGGING.getAsBoolean()) {
                if (request == null) {
                    ...
                    String type = request.getRequest() == null ? "<null>" : request.getRequest().getClass().getName();
                    ...
                    if (assignmentStore != null) {
                        ...
                        if (assignedToken != null) {
                            try {
                                ...
                                assignedInfo = assignedToken + " -> " + assignedResolver.getClass().getName()
                                        + " (priority=" + assignedResolver.getPriority() + ")";
                            } catch (IllegalArgumentException ex) {
                        ...
                        if (!snapshot.equals(last)) {
                            TheSettlerXCreate.LOGGER.info(
                                    "[CreateShop] request {} type={} state={}",
                                    token, type, state);
                            TheSettlerXCreate.LOGGER.info(
                                    "[CreateShop] request {} assigned resolver {}",
                                    token, assignedInfo);
                        }
                    } else {
            ...
            if (Config.DEBUG_LOGGING.getAsBoolean() && request != null) {
                ...
                if (payload instanceof com.minecolonies.api.colony.requestsystem.requestable.deliveryman.Delivery
                        || payload instanceof com.minecolonies.api.colony.requestsystem.requestable.deliveryman.IDeliverymanRequestable) {
            ...
            reassignCount += tryReassignRequest(token, request, gameTime, resolverHandler, requestHandler, assignmentStore);
        }
        if (reassignCount > 0 && Config.DEBUG_LOGGING.getAsBoolean()) {
            TheSettlerXCreate.LOGGER.info("[CreateShop] Reassigned {} deliverable requests for CreateShop", reassignCount);
        }
        if (Config.DEBUG_LOGGING.getAsBoolean()
                ...
                for (var resolverToken : requestableList) {
                    try {
                        ...
                        boolean matches = requestType != null
                                && (requestType.isAssignableFrom(com.minecolonies.api.colony.requestsystem.requestable.deliveryman.Delivery.class)
                                || requestType.isAssignableFrom(com.minecolonies.api.colony.requestsystem.requestable.deliveryman.IDeliverymanRequestable.class)
                                || requestType.isAssignableFrom(com.minecolonies.api.colony.requestsystem.requestable.IRequestable.class));
                        if (matches) {
                            matchingResolvers++;
                        ...
                        if (resolver instanceof com.minecolonies.core.colony.requestsystem.resolvers.DeliveryRequestResolver) {
                            ...
                            var assignments = resolverAssignmentStore == null ? null : resolverAssignmentStore.getAssignments();
    ...
    private static int tryReassignRequest(
            ...
            com.minecolonies.api.colony.requestsystem.data.IRequestResolverRequestAssignmentDataStore assignmentStore
    ) {
        if (request == null) {
            return 0;
        }
        ...
        if (request.hasChildren()) {
            int reassigned = 0;
            if (Config.DEBUG_LOGGING.getAsBoolean()) {
                TheSettlerXCreate.LOGGER.info("[CreateShop] request {} has {} child(ren)", token, request.getChildren().size());
            }
            for (var childToken : request.getChildren()) {
                ...
                if (Config.DEBUG_LOGGING.getAsBoolean()) {
                    String childType = child == null || child.getRequest() == null ? "<null>" : child.getRequest().getClass().getName();
                    String childState = child == null ? "<null>" : String.valueOf(child.getState());
                    TheSettlerXCreate.LOGGER.info("[CreateShop] child {} type={} state={}", childToken, childType, childState);
                }
                reassigned += tryReassignRequest(childToken, child, gameTime, resolverHandler, requestHandler, assignmentStore);
            }
        ...
        if (Config.DEBUG_LOGGING.getAsBoolean()) {
            String type = request.getRequest() == null ? "<null>" : request.getRequest().getClass().getName();
            TheSettlerXCreate.LOGGER.info("[CreateShop] inspect request {} type={} state={}", token, type, request.getState());
        }
        ...
        if (assignmentStore != null) {
            ...
            if (assignedToken != null) {
                try {
                    var assignedResolver = resolverHandler.getResolver(assignedToken);
                    assignedToRetrying = assignedResolver
                            instanceof com.minecolonies.core.colony.requestsystem.resolvers.StandardRetryingRequestResolver;
                    assignedToPlayer = assignedResolver
                            instanceof com.minecolonies.core.colony.requestsystem.resolvers.StandardPlayerRequestResolver;
                } catch (IllegalArgumentException ignored) {
        ...
        Object payload = request.getRequest();
        boolean isDeliverable = payload instanceof com.minecolonies.api.colony.requestsystem.requestable.IDeliverable;
        boolean isDeliveryRequest = payload instanceof com.minecolonies.api.colony.requestsystem.requestable.deliveryman.Delivery
                || payload instanceof com.minecolonies.api.colony.requestsystem.requestable.deliveryman.IDeliverymanRequestable;
        if (state == com.minecolonies.api.colony.requestsystem.request.RequestState.IN_PROGRESS
                ...
                || state == com.minecolonies.api.colony.requestsystem.request.RequestState.FOLLOWUP_IN_PROGRESS) {
            if (Config.DEBUG_LOGGING.getAsBoolean() && isDeliveryRequest) {
                TheSettlerXCreate.LOGGER.info("[CreateShop] skip reassign {} (delivery state={} assigned={})",
                        token, state, assignedToken == null ? "<none>" : assignedToken);
            }
            return 0;
        }
        ...
        try {
            ...
            if (Config.DEBUG_LOGGING.getAsBoolean()) {
                TheSettlerXCreate.LOGGER.info("[CreateShop] reassigned request {} (assigned={})", token, assigned);
                if (isDeliveryRequest && assignmentStore != null) {
                    ...
                    if (postAssignToken != null) {
                        try {
                            ...
                            TheSettlerXCreate.LOGGER.info("[CreateShop] delivery {} now assigned to {} -> {}",
                                    token, postAssignToken, assignedResolver.getClass().getName());
                        } catch (IllegalArgumentException ex) {
                            TheSettlerXCreate.LOGGER.info("[CreateShop] delivery {} now assigned to {} -> <missing>",
                                    token, postAssignToken);
                        }
                    } else {
                        TheSettlerXCreate.LOGGER.info("[CreateShop] delivery {} still unassigned after reassign", token);
                    }
                }
            }
            return 1;
            ...
            if (Config.DEBUG_LOGGING.getAsBoolean()) {
                TheSettlerXCreate.LOGGER.info("[CreateShop] reassign failed {}: {}", token, ex.getMessage());
            }
            return 0;
        }
    }
        ...
        for (var childToken : children) {
            ...
            if (child.getChildren().contains(token)) {
                ...
                if (Config.DEBUG_LOGGING.getAsBoolean()) {
                    ...
                    if (CHILD_CYCLE_LOGGED.add(key)) {
                        TheSettlerXCreate.LOGGER.info(
                                "[CreateShop] removed child-parent cycle parent={} child={}",
                                token,
                                childToken);
                    }
                }
    ...
    private static void sanitizeAllRequestChains(
        ...
        java.util.Set<com.minecolonies.api.colony.requestsystem.token.IToken<?>> visited = new java.util.HashSet<>();
        int removedCycles = 0;
        ...
        if (Config.DEBUG_LOGGING.getAsBoolean() && removedCycles > 0) {
            TheSettlerXCreate.LOGGER.info("[CreateShop] sanitized request chains removedCycles={}", removedCycles);
        }
    }
    ...
    private static int sanitizeRequestChain(
        ...
        java.util.Set<com.minecolonies.api.colony.requestsystem.token.IToken<?>> visiting = new java.util.HashSet<>();
        java.util.ArrayDeque<com.minecolonies.api.colony.requestsystem.request.IRequest<?>> stack =
                new java.util.ArrayDeque<>();
        java.util.ArrayDeque<java.util.Iterator<com.minecolonies.api.colony.requestsystem.token.IToken<?>>> itStack =
                new java.util.ArrayDeque<>();
        stack.push(root);
        itStack.push(root.getChildren() == null
                ? java.util.Collections.<com.minecolonies.api.colony.requestsystem.token.IToken<?>>emptyList().iterator()
    ...
    private static String buildDeliveryDebugDump(
        ...
        return "token=" + token + " state=" + state + " assigned=" + assigned
                + " requester=" + requester + " pickup=" + pickup + " delivery=" + delivery
                + " stack=" + stack;
    }
